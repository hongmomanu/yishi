...
class app
{
	...
	public function basic()
	{
		...
		switch($subaction)
		{
			...
			if($this->ev->get('insertsection'))
			{
				...
				else
				{
					...
					$message = array(
						...
						"message" => "操作成功",
						"callbackType" => "forward",
					    "forwardUrl" => "index.php?exam-teach-basic-section&subjectid={$args['sectionsubjectid']}"
					);
				}
			...
			if($this->ev->get('modifysection'))
			{
				...
				if($tpsection)
				{
					$message = array(
					    ...
					    "forwardUrl" => "index.php?exam-teach-basic-section&subjectid={$section['sectionsubjectid']}&page={$page}"
					);
				}
				else
				{
			...
			{
				header('location:index.php?exam-teach-subject');
				exit;
			}
			else
			{
			...
			if($this->ev->get('insertknows'))
			{
				...
				$message = array(
					'statusCode' => 200,
					"message" => "操作成功！".$errmsg,
				    "forwardUrl" => "index.php?exam-teach-basic-point&sectionid={$args['knowssectionid']}"
				);
				exit(json_encode($message));
			}
			else
			...
			if($this->ev->get('modifypoint'))
			{
				...
				else
				{
					...
					$message = array(
						...
						"message" => "操作成功",
						"callbackType" => "forward",
					    "forwardUrl" => "index.php?exam-teach-basic-point&sectionid={$knows['knowssectionid']}&page={$page}"
					);
				}
				exit(json_encode($message));
			}
			...
			if($this->ev->get('insertsubject'))
			{
				...
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "index.php?exam-teach-basic-subject"
				);
				exit(json_encode($message));
			}
			else
			...
			if($this->ev->get('insertbasic'))
			{
				...
				if($id){
					$args = array('user');
					$data = array('userid',$args,"1");
					$sql = $this->sql->makeSelect($data);
					$basicid = $this->db->fetchAll($sql);
					if($basicid){
						foreach($basicid as $item){
							$args = array(
									'obuserid' => $item['userid'],
									'obbasicid' => $id,
									'obtime' => time(),
									'obendtime' => time()+3600*24*365*3
							);
			...
			$subjects = $this->basic->getSubjectList();
			$subid = array();
			foreach($subjects as $sbk=>$sbv){
				array_push($subid,$sbv['subjectid']);
			}
			$nsubid = implode(',',$subid);
			$args = array("basicsubjectid IN ({$nsubid})");
