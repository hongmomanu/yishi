...
function showHide(id){
	...
	if(name == 'none'){
        ...
        <div class="ny_R right">
        	<div  class="neirong">
            	<ul class="dongtai">
            	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$msg): $mod = ($i % 2 );++$i;?><li><span class="left"><?php if($msg['from__uid'] == is_login()){ ?>发送给<?php echo (getuserinfo($msg["to_uid"],uname)); }else{ ?>来自<?php echo (getuserinfo($msg["from_uid"],uname)); } ?><a href="<?php echo U('Member/messageview',array('mid'=>$msg['mid']));?>">· <?php echo ($msg["title"]); ?></a></span><span class="right">[<?php echo (date('Y-m-d H:i:s',$msg["date"])); ?>]</span></li><?php endforeach; endif; else: echo "" ;endif; ?>
            </ul>
