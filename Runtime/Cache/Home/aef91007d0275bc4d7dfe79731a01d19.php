...
<input type="hidden" name="action" value="editsave" /> <!--add replace by editsave-->
<input type="hidden" name="pid" value="<?php echo ($pid); ?>" />
<ul>
<li class="text"><span>培训标题</span><input name="title" type="text" value="<?php echo ($fdata["title"]); ?>"/><b style="color:#F00">*</b></li>
<li class="text"><span>类型</span><select name="pxtype" id="pxtype" onchange="changeon()"><option value="1" <?php echo (retrunselect($fdata["pxtype"],1)); ?>>培训课件</option><option value="2" <?php echo (retrunselect($fdata["pxtype"],2)); ?>>考试</option></select></li>
<li class="text" id="examid" <?php if($fdata['pxtype'] == 1): ?>style="display:none;"<?php endif; ?>><span>试题</span>
<select name="examid">
<option value="0">请选择</option>
<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$exam): $mod = ($i % 2 );++$i;?><option value="<?php echo ($exam["basicid"]); ?>" <?php echo (retrunselect($fdata["examid"],$exam['basicid'])); ?>><?php echo ($exam["basic"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select><b style="color:#F00">*</b></li>
<li class="text" id="fileupload" <?php if($fdata['pxtype'] == 2): ?>style="display:none;"<?php endif; ?>><span>上传文件</span><input type="file" name="images" /><b style="color:#F00">*</b></li>
