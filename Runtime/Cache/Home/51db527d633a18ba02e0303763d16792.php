...
function Edit(id)
{
	location.href='<?php echo U('Member/articleedit');?>/id/'+ id;
}
function View(uid)
{
	location.href='<?php echo U('Index/article');?>/id/'+ uid;
}
function Del(id)
{
	if (confirm("确定删除该记录？请慎重操作！"))
	{
		location.href='<?php echo U('Member/articledel');?>/id/'+ id;
	}
}
function Top(id){
	if (confirm("确定置顶该公告？请慎重操作！"))
	{
		location.href='<?php echo U('Member/articletop');?>/id/'+ id;
	}
}
function Search(){
	var name = document.getElementById('UserName').value;
	location.href="<?php echo U('Member/article');?>/title/"+name;
}
</script>
</head>
...
<a href="<?php echo U('Member/message');?>"><li>站内消息<?php echo getnewMessagecount(); ?></li></a>
...
<a onClick='showHide("items2_5")'><li>文章管理</li></a>
<dl id="items2_5" <?php if(CONTROLLER_NAME == 'News'){ }else{ ?>style="display:none;"<?php } ?>>
<dd><a href="<?php echo U('News/index');?>">·文章列表</a></dd>
<dd><a href="<?php echo U('News/newsadd');?>">·新增文章</a></dd>
</dl>
...
<td nowrap='nowrap'>发布者</td>
	<td nowrap='nowrap'>发布时间</td>
	<td nowrap='nowrap'>点击量</td>
	<td nowrap='nowrap'>头条</td>
...
<td nowrap='nowrap'><?php echo (getuserinfo($user["uid"],uname)); ?></td>
<td nowrap='nowrap'><?php echo (date('Y-m-d H:i:s',$user["posttime"])); ?></td>
<td nowrap='nowrap'><?php echo ($user["hits"]); ?></td>
<td nowrap='nowrap'><?php if($user['istop'] == 1): ?><span style="color:red">是</span><?php else: ?><a href="javascript:void(0)" onclick="Top(<?php echo ($user["id"]); ?>)">否</a><?php endif; ?></td>
<td nowrap='nowrap'><?php if($user['status'] == 1): ?>已发布<?php elseif($user['status'] == 2): ?>驳回<?php elseif($user['status'] == 3): ?>已删除<?php else: ?>待审核<?php endif; ?></td>
<td nowrap='nowrap'>
<?php if(session('groupid') == 1){ ?>
<span style="cursor:pointer;" title="编辑" onclick="View(<?php echo ($user["id"]); ?>,0)">[查看]</span>
<span style="cursor:pointer;" title="编辑" onclick="Edit(<?php echo ($user["id"]); ?>,1)">[修改]</span>
<span style="cursor:pointer;" title="删除" onclick="Del(<?php echo ($user["id"]); ?>)">[删除]</span>
<?php }else{ ?>
<?php if($user['status'] == 1): ?><span style="cursor:pointer;" title="编辑" onclick="View(<?php echo ($user["id"]); ?>,0)">[查看]</span>
<?php elseif($user['status'] == 2): ?>
<span style="cursor:pointer;" title="编辑" onclick="Edit(<?php echo ($user["id"]); ?>,1)">[修改]</span><?php endif; ?>
<span style="cursor:pointer;" title="删除" onclick="Del(<?php echo ($user["id"]); ?>)">[删除]</span>
<?php } ?>
</td>
