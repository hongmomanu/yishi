...
function View(id)
{
	if (confirm("确定发布该公告？请慎重操作！"))
	{
		location.href='<?php echo U('Member/articlechangestatus');?>/id/'+ id +'/type/fabu';
	}
}
function Edit(id)
{
	if (confirm("确定驳回该公告？请慎重操作！"))
	{
		var ret = window.showModalDialog("<?php echo U('Member/bohui');?>/id/"+id,null,"dialogWidth:800px;dialogHeight:500px;help:no;status:no");
		 alert(ret);
...
function Search(){
	...
	location.href="<?php echo U('Member/articlestatus');?>/title/"+name;
}
...
<td nowrap='nowrap'>
<span style="cursor:pointer;" title="编辑" onclick="Edit(<?php echo ($user["id"]); ?>,1)">[驳回]</span>
<span style="cursor:pointer;" title="删除" onclick="View(<?php echo ($user["id"]); ?>)">[通过]</span>
</td>
