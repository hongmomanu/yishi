function showHide(objname)
{
  ...
  //正常设置cookie
	...
	var okstr ='';
	var ischange = false;
	var tds1 = document.getElementById('diov1').getElementsByTagName('b');
	if(ckstr==null) ckstr = '';
	ckstrs = ckstr.split(',');
	...
	else
	{
			...
			for (var i = 1 ; i < 7 ; i++)
			{
				var objs = document.getElementById('items'+i+'_1');	
				if ( i+'_1' !== objname)
				{
					objs.style.display = 'none';
					tds1[j].className = '';
				}else{
					tds1[j].className = 'tdcolor';
				}
				j++;
			}
	}
...
var tds = document.getElementById('diov1').getElementsByTagName('a');
...
function aonclick(obj,obj1){
	for( var o=0; o<obj1.length; o++ ){
		if( obj1[o] == obj ){
			obj1[o].className = 'tdcolor2';
		}
		else{
			obj1[o].className = '';
		}
